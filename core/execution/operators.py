...
#         event_val = event[field]
#         if op == '>': return event_val > val
#         elif op == '<': return event_val < val
#         elif op == '=': return event_val == val
#         elif op == '>=': return event_val >= val
#         elif op == '<=': return event_val <= val
#         elif op == '!=': return event_val != val
...
#         passed = [e for e in events if self._filter_one(e)]

#         if passed and self.next_op:
#             self.next_op.process(passed)
...
#     def _project_one(self, event):
#         if self.select_list == '*' or not isinstance(self.select_list, list):
#             return event
#         projected = {}
#         for item in self.select_list:
#             if isinstance(item, str) and item in event:
...
#         projected = [self._project_one(e) for e in events]
#         self.next_op.process(projected)
...
#             # Collect all result events, then flush them all at once to next_op
#             result_events = []
...
#             # Flush all aggregated results at once as a batch
#             if self.next_op and result_events:
#                 self.next_op.process(result_events)
...
#                 elapsed = now - self.window_start_time
#                 if elapsed >= self.window_seconds:
#                     # Collect the full cross-join of both buffers before clearing
#                     outputs = self._join_all_buffers()
...
#                     self._clear_all_db_state()
#             else:
#                 # SLIDING: evict expired events, then buffer the new one,
#                 # then compute all matches for this event — but don't emit yet;
#                 # collect into outputs and flush below outside the lock.
#                 self._evict_old(now)
...
#         # Flush the entire batch of matched events at once as a list,
#         # consistent with how WindowOperator.emit_window() sends a list.
#         if outputs:
#             self.next_op.process(outputs)
...
#         # Accept either a single event dict or a list (batch from upstream window/join)
#         if not isinstance(events, list):
#             events = [events]
...
#         # Collect ALL joined rows across every input event, then flush once
#         result_events = []
#         for event in events:
#             if self.left_field not in event:
#                 continue
...
#             # INNER JOIN: only emit when at least one matching table row exists
#             for row in rows:
#                 joined_event = dict(event)
#                 row_dict = dict(row)
#                 for key, val in row_dict.items():
...
#                     else:
#                         joined_event[key] = val
#                 result_events.append(joined_event)

#         # Push the entire batch downstream at once
#         if result_events:
#             self.next_op.process(result_events)
...
#     def process(self, events):
#         # SinkOperator is the terminal node — it unpacks the batch and
#         # emits each event individually to the target (Kafka, table, callback).
#         if not isinstance(events, list):
#             events = [events]

#         for event in events:
#             self._emit_one(event)
...
class JoinOperator(Operator):
    ...
    def process(self, events):
        ...
        sql_op = op_map.get(self.operator)
        if sql_op is None:
            print(f"[JOIN] Unsupported operator '{self.operator}'")
            return

        result_events = []
        for event in events:
            if self.left_field not in event:
                print(f"[JOIN] DROP — left_field '{self.left_field}' missing. "
                      f"Event keys: {list(event.keys())}", flush=True)
                continue
            ...
            try:
                ...
                print(f"[JOIN] SQL error on table '{self.table_name}': {e}", flush=True)
                continue

            if not rows:
                try:
                    sample = conn.execute(
                        f'SELECT "{self.right_field}" FROM "{self.table_name}" LIMIT 5'
                    ).fetchall()
                    sample_vals = [r[0] for r in sample]
                except Exception:
                    sample_vals = ['<could not sample>']
                print(f"[JOIN] NO MATCH — table='{self.table_name}' "
                      f"right_field='{self.right_field}' "
                      f"join_value={join_value!r} (type={type(join_value).__name__}) "
                      f"sample_table_vals={sample_vals}", flush=True)
                continue
